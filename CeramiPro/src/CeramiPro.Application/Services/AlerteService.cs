...
public class AlerteService : IAlerteService
{
    ...
    public AlerteService(IApplicationDbContext context, IServiceDateHeure horloge)
    {
        _context = context;
        _horloge = horloge;
    }
    ...
    public async Task MarquerLueAsync(int id, CancellationToken cancellationToken = default)
    {
        var alerte = await _context.Notifications.FirstOrDefaultAsync(a => a.Id == id, cancellationToken)
                     ?? throw IntrouvableException.Pour("Alerte", id);

        if (!alerte.IsRead)
        {
            alerte.IsRead = true;
            alerte.ReadAt = _horloge.MaintenantUtc;
        ...
        int id, ReglageAlerteDto reglage, CancellationToken cancellationToken = default)
    {
        var existant = await _context.NotificationSettings
                       ...
                       ?? throw IntrouvableException.Pour("Réglage d'alerte", id);

        if (reglage.SeuilJours is < 0 or > 365)
        {
            throw new RegleMetierException("Le délai d'alerte doit être compris entre 0 et 365 jours.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return produits.Select(p => Creer(
            ...
            p.CurrentStock <= 0
                ...
                : $"Il reste {Formatage.Quantite(p.CurrentStock, "pièce")} " +
                  $"pour un minimum de {Formatage.Quantite(p.MinimumStock, "pièce")}.",
            "Product", p.Id,
        ...
        CancellationToken cancellationToken)
    {
        ...
        return matieres.Select(m => Creer(
            ...
            m.CurrentQuantity <= 0
                ? "Le stock est épuisé."
                : $"Il reste {Formatage.Quantite(m.CurrentQuantity, m.Unite)} " +
                  $"pour un minimum de {Formatage.Quantite(m.MinimumStock, m.Unite)}.",
            "Material", m.Id,
        ...
        if (Actif(reglages, NotificationType.ProductionRetard))
        {
            alertes.AddRange(ordres
                ...
                .Select(o => Creer(
                    ...
                    $"{Formatage.Date(o.PlannedEndDate!.Value)}.",
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (Actif(reglages, NotificationType.DetteClient))
        {
            ...
            alertes.AddRange(clients.Where(c => c.Reste > seuil && c.Reste > 0).Select(c => Creer(
                ...
                $"{Formatage.Montant(c.Reste)} restent à encaisser.",
                "Customer", c.Id,
                "clients/dettes")));
        }
        ...
        if (Actif(reglages, NotificationType.DetteFournisseur))
        {
            ...
            alertes.AddRange(achats.Where(a => a.Reste > seuil).Select(a => Creer(
                ...
                $"{Formatage.Montant(a.Reste)} restent dus à {a.Fournisseur}.",
                "Purchase", a.Id,
    ...
    private Notification Creer(
        ...
        => new()
        {
            ...
            EntityId = entiteId,
            Link = adresse,
            CreatedAt = _horloge.MaintenantUtc
        };
