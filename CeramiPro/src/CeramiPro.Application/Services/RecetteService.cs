...
public class RecetteService : IRecetteService
{
    ...
    public async Task<RecetteDto> ObtenirAsync(int id, CancellationToken cancellationToken = default)
    {
        var recette = await ChargerAvecDetails().AsNoTracking()
                      ...
                      ?? throw IntrouvableException.Pour("Recette", id);
        ...
        int recetteId, decimal quantite, CancellationToken cancellationToken = default)
    {
        if (quantite <= 0)
        {
            throw new RegleMetierException("Indiquez le nombre de pièces à produire.");
        }

        var recette = await ChargerAvecDetails().AsNoTracking()
                          .FirstOrDefaultAsync(r => r.Id == recetteId, cancellationToken)
                      ?? throw IntrouvableException.Pour("Recette", recetteId);
    ...
    private async Task VerifierAsync(RecetteRequete requete, CancellationToken cancellationToken)
    {
        ...
        {
            throw new RegleMetierException("Le nombre de pièces obtenues doit être supérieur à zéro.");
        }

        if (requete.Lignes.Count == 0)
        {
            throw new RegleMetierException("Ajoutez au moins une matière à la recette.");
        }

        if (requete.Lignes.Select(l => l.MatiereId).Distinct().Count() != requete.Lignes.Count)
        {
            throw new RegleMetierException("Une même matière ne peut apparaître qu'une seule fois dans la recette.");
        }
        ...
        foreach (var ligne in requete.Lignes)
        {
            if (ligne.Quantite <= 0)
            {
                throw new RegleMetierException("Chaque matière doit avoir une quantité supérieure à zéro.");
            }
