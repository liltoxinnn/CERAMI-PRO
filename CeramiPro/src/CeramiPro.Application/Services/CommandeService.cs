...
public class CommandeService : ICommandeService
{
    ...
    public async Task<CommandeDto> ObtenirAsync(int id, CancellationToken cancellationToken = default)
    {
        var commande = await ChargerAvecDetails().AsNoTracking()
                       ...
                       ?? throw IntrouvableException.Pour("Commande personnalisée", id);
        ...
        CommandeRequete requete, CancellationToken cancellationToken = default)
    {
        ...
        var commande = new CustomOrder
        {
            ...
            OrderDate = _horloge.MaintenantUtc,
        ...
        await _audit.EnregistrerAsync(AuditAction.Creation, nameof(CustomOrder), commande.Id.ToString(),
            ...
            $"({Formatage.Montant(commande.TotalAmount)}).", null, cancellationToken);
        ...
        int id, CommandeRequete requete, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new RegleMetierException(
                $"Le nouveau total ({Formatage.Montant(nouveauTotal)}) est inférieur au montant " +
                $"déjà encaissé ({Formatage.Montant(commande.PaidAmount)}).");
        }
        ...
        int id, CustomOrderStatus statut, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new RegleMetierException($"La commande {commande.OrderNumber} est annulée.");
        }
        ...
        {
            throw new RegleMetierException(
                $"Il reste {Formatage.Montant(commande.RemainingAmount)} à encaisser " +
        ...
        {
            commande.DeliveredAt = _horloge.MaintenantUtc;
        }
        ...
        int id, string motif, CancellationToken cancellationToken = default)
    {
        ...
        if (commande.Status == CustomOrderStatus.Annule)
        {
            throw new RegleMetierException($"La commande {commande.OrderNumber} est déjà annulée.");
        }
        ...
        {
            throw new RegleMetierException(
                $"Un acompte de {Formatage.Montant(commande.PaidAmount)} a été encaissé. " +
    ...
    private async Task VerifierAsync(CommandeRequete requete, CancellationToken cancellationToken)
    {
        ...
        {
            throw new RegleMetierException("Décrivez brièvement la pièce demandée.");
        }

        if (requete.Quantite <= 0)
        {
            throw new RegleMetierException("La quantité doit être supérieure à zéro.");
        }

        if (requete.PrixUnitaire < 0 || requete.Remise < 0)
        {
            throw new RegleMetierException("Le prix et la remise ne peuvent pas être négatifs.");
        }

        // Règle métier n°16 : une commande personnalisée a toujours une date limite.
        if (requete.DateLimite is null)
        {
            throw new RegleMetierException("La date limite de la commande est obligatoire.");
        }
    }
