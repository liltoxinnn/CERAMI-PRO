...
namespace CeramicWorkshop.UnitTests.Codes;
...
public class CodeServiceTests : IDisposable
{
    ...
    public async Task Une_planche_sans_produit_est_refusee()
    {
        ...
        await action.Should().ThrowAsync<BusinessRuleException>().WithMessage("*au moins un produit*");
    }
    ...
    public async Task Une_planche_trop_grande_est_refusee()
    {
        ...
        await action.Should().ThrowAsync<BusinessRuleException>().WithMessage("*étiquettes*");
    }
