...
public class ProductionService : IProductionService
{
    ...
    public async Task<OrdreProductionDto> ObtenirAsync(int id, CancellationToken cancellationToken = default)
    {
        var ordre = await ChargerAvecDetails().AsNoTracking()
                    ...
                    ?? throw NotFoundException.Pour("Ordre de production", id);
        ...
        OrdreProductionRequete requete, CancellationToken cancellationToken = default)
    {
        ...
        var ordre = new ProductionOrder
        {
            ...
            PlannedStartDate = requete.DateDebutPrevue ?? _horloge.UtcNow,
        ...
        await _audit.EnregistrerAsync(AuditAction.Creation, nameof(ProductionOrder), ordre.Id.ToString(),
            ...
            $"({MontantFormatter.FormaterQuantite(ordre.PlannedQuantity)} pièce(s)).", null, cancellationToken);
        ...
        int id, LancementProductionRequete requete, CancellationToken cancellationToken = default)
    {
        ...
        if (ordre.Status != ProductionStatus.Planifie)
        {
            throw new BusinessRuleException($"La production {ordre.ProductionNumber} est déjà lancée.");
        }
        ...
        var manquants = ordre.Materials
            ...
            .Select(m =>
                ...
                $"{MontantFormatter.FormaterQuantite(m.PlannedQuantity, m.Material.Unit.Code)}, " +
                $"disponible : {MontantFormatter.FormaterQuantite(m.Material.CurrentQuantity, m.Material.Unit.Code)}")
            .ToList();
        ...
        if (manquants.Count > 0)
        {
            ...
            if (!derogation)
            {
                throw new BusinessRuleException("Matières insuffisantes pour lancer cette production.", manquants);
            }

            if (string.IsNullOrWhiteSpace(requete.MotifDerogation))
            {
                throw new BusinessRuleException("Indiquez le motif de la dérogation.");
            }

            ordre.StockCheckOverridden = true;
            ordre.OverriddenByUserId = _utilisateurCourant.UserId;
        ...
        ordre.ActualStartDate = _horloge.UtcNow;
        ordre.Status = ProductionStatus.Preparation;

        AjouterEtape(ordre, ProductionStatus.Preparation, ordre.PlannedQuantity, 0m,
            _utilisateurCourant.UserId, "Lancement de la production et consommation des matières.");

        await _context.SaveChangesAsync(cancellationToken);

        await _audit.EnregistrerAsync(AuditAction.Modification, nameof(ProductionOrder), id.ToString(),
            $"Lancement de la production {ordre.ProductionNumber} : " +
            $"{MontantFormatter.Formater(ordre.ActualMaterialCost)} de matières consommées." +
        ...
        int id, ChangementEtapeRequete requete, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new BusinessRuleException("Les quantités ne peuvent pas être négatives.");
        }
        ...
        if (requete.QuantiteEndommagee > restantes)
        {
            throw new BusinessRuleException(
                ...
                $"({MontantFormatter.FormaterQuantite(restantes)}).");
        }
        ...
        {
            etapeOuverte.EndedAt = _horloge.UtcNow;
        ...
        else
        {
            AjouterEtape(ordre, requete.NouvelleEtape, 0m, 0m,
                requete.EmployeId ?? _utilisateurCourant.UserId, Nettoyer(requete.Notes));
        }
        ...
        ProductionOrder ordre, ChangementEtapeRequete requete, CancellationToken cancellationToken)
    {
        ...
        ordre.ActualEndDate = _horloge.UtcNow;
        ...
        if (acceptees > 0)
        {
            ...
            await _inventaire.EnregistrerAsync(new MouvementStockRequete
            {
                ...
                ProductionId = ordre.Id,
                Reference = ordre.ProductionNumber,
                Notes = $"Production terminée : {MontantFormatter.FormaterQuantite(acceptees)} pièce(s)."
            }, cancellationToken);
        ...
        int id, string motif, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new BusinessRuleException($"La production {ordre.ProductionNumber} est déjà annulée.");
        }
    ...
    private void AjouterEtape(
        ...
        {
            Stage = etape,
            StartedAt = _horloge.UtcNow,
            EndedAt = etape == ProductionStatus.Termine ? _horloge.UtcNow : null,
            UserId = employeId,
    ...
    private async Task VerifierAsync(OrdreProductionRequete requete, CancellationToken cancellationToken)
    {
        ...
        {
            throw new BusinessRuleException("La quantité à produire doit être supérieure à zéro.");
        }
        ...
        {
            throw new BusinessRuleException("La recette sélectionnée ne correspond pas à ce produit.");
        }

        if (requete.DateFinPrevue is not null && requete.DateDebutPrevue is not null
            && requete.DateFinPrevue < requete.DateDebutPrevue)
        {
            throw new BusinessRuleException("La date de fin prévue doit suivre la date de début.");
        }
    }
    ...
    private OrdreProductionDto Convertir(ProductionOrder o) => new(
        ...
        o.OverrideReason,
        o.PlannedEndDate is not null && o.PlannedEndDate < _horloge.UtcNow
        && o.Status != ProductionStatus.Termine && o.Status != ProductionStatus.Annule,
