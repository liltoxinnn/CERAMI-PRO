...
public class AchatService : IAchatService
{
    ...
    public async Task<AchatDto> ObtenirAsync(int id, CancellationToken cancellationToken = default)
    {
        var achat = await ChargerAvecDetails().AsNoTracking()
                    ...
                    ?? throw NotFoundException.Pour("Achat", id);
    ...
    public async Task<AchatDto> CreerAsync(AchatRequete requete, CancellationToken cancellationToken = default)
    {
        ...
        var achat = new Purchase
        {
            ...
            PurchaseDate = requete.Date ?? _horloge.UtcNow,
        ...
        await _audit.EnregistrerAsync(AuditAction.Creation, nameof(Purchase), achat.Id.ToString(),
            ...
            $"({MontantFormatter.Formater(achat.TotalAmount)}).", null, cancellationToken);
    ...
    public async Task<AchatDto> ConfirmerAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        if (achat.Status != PurchaseStatus.Brouillon)
        {
            throw new BusinessRuleException($"L'achat {achat.PurchaseNumber} est déjà confirmé.");
        }

        if (achat.Items.Count == 0)
        {
            throw new BusinessRuleException("Ajoutez au moins une matière avant de confirmer l'achat.");
        }
        ...
        int id, ReceptionAchatRequete requete, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new BusinessRuleException("Confirmez l'achat avant d'enregistrer une réception.");
        }
        ...
        {
            throw new BusinessRuleException("Indiquez les quantités reçues.");
        }
        ...
        foreach (var ligneRecue in requete.Lignes.Where(l => l.QuantiteRecue > 0))
        {
            var ligne = achat.Items.FirstOrDefault(i => i.Id == ligneRecue.LigneAchatId)
                        ?? throw new BusinessRuleException("Une ligne de réception ne correspond à aucune ligne d'achat.");
            ...
            if (ligneRecue.QuantiteRecue > restant)
            {
                throw new BusinessRuleException(
                    ...
                    $"Restant à recevoir : {MontantFormatter.FormaterQuantite(restant, ligne.Unit.Code)}.");
            }
        ...
        int id, string motif, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new BusinessRuleException($"L'achat {achat.PurchaseNumber} est déjà annulé.");
        }
        ...
        if (achat.PaidAmount > 0)
        {
            throw new BusinessRuleException(
                ...
                $"({MontantFormatter.Formater(achat.PaidAmount)}). " +
    ...
    private async Task VerifierRequeteAsync(AchatRequete requete, CancellationToken cancellationToken)
    {
        ...
        {
            throw new BusinessRuleException("Le fournisseur sélectionné n'existe pas.");
        }

        if (requete.Lignes.Count == 0)
        {
            throw new BusinessRuleException("Ajoutez au moins une matière à l'achat.");
        }
        ...
        foreach (var ligne in requete.Lignes)
        {
            if (ligne.Quantite <= 0)
            {
                throw new BusinessRuleException("Chaque ligne doit avoir une quantité supérieure à zéro.");
            }

            if (ligne.PrixUnitaire < 0)
            {
                throw new BusinessRuleException("Le prix unitaire ne peut pas être négatif.");
            }
        ...
        {
            throw new BusinessRuleException("La remise et les frais de livraison ne peuvent pas être négatifs.");
        }
    }
