...
public class VenteService : IVenteService
{
    ...
    public async Task<VenteDto> ObtenirAsync(int id, CancellationToken cancellationToken = default)
    {
        var vente = await ChargerAvecDetails().AsNoTracking()
                    ...
                    ?? throw NotFoundException.Pour("Vente", id);
        ...
        VenteRequete requete, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new BusinessRuleException("Ajoutez au moins un produit à la vente.");
        }
        ...
        var vente = new Sale
        {
            ...
            SaleDate = requete.Date ?? _horloge.UtcNow,
        ...
        if (requete.MontantPaye > 0)
        {
            ...
            {
                throw new BusinessRuleException("Choisissez le mode de règlement.");
            }
        ...
        await _audit.EnregistrerAsync(AuditAction.Creation, nameof(Sale), vente.Id.ToString(),
            $"Vente {vente.SaleNumber} de {MontantFormatter.Formater(vente.TotalAmount)}.",
        ...
        int id, string motif, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new BusinessRuleException($"La vente {vente.SaleNumber} est déjà annulée.");
        }
        ...
        if (vente.PaidAmount > 0)
        {
            throw new BusinessRuleException(
                ...
                $"({MontantFormatter.Formater(vente.PaidAmount)}). Annulez d'abord les paiements.");
        }
